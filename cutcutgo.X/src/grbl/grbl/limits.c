...
volatile uint8_t g_hard_limits = 0;
bool g_hard_limits_enabled = false;

void limits_init()
{
    /* Initialize hardware limits. */
    g_hard_limits = 0;
    g_hard_limits_enabled = true;
}
...
void limits_set_state(uint8_t axis, bool triggered)
{
    if (axis < N_AXIS)
    {
        if (triggered)
        {
            /* Update hard limit state. */
            g_hard_limits |= (1 << axis);
            
            /* If enabled, process limit trigger. */
            if (g_hard_limits_enabled)
            {
