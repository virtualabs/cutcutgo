...
void mc_line(float *target, plan_line_data_t *pl_data)
{
  ...
  if (sys.state == STATE_CHECK_MODE) {
    //printString("[mc_line(): check mode enabled]\r\n");
    return;
  }
  ...
  //printString("[mc_line(): start do loop]\r\n");
  do {
  ...
  //printString("[mc_line(): exit do loop]\r\n");

  // Plan and queue motion into planner buffer
  //printString("[mc_line(): plan_buffer_line]\r\n");
  if (plan_buffer_line(target, pl_data) == PLAN_EMPTY_BLOCK) {
    //printString("[mc_line(): plan_buffer_line()==PLAN_EMPTY_BLOCK]");
