 ...
 * - On fait un mix:
 * 
 *   -> pour les segments verticaux/horizontaux: on asservit simplement en position (moteur fait X pas)
 *
...
typedef struct {
    /* Data for Bresenham algorithm tracer. */
...
static stepper_t stepper_info;
static uint8_t selected_tool = 0;
...
void st_select_tool(uint8_t tool)
{
    /* Keep track of current tool. */
    selected_tool = tool;
    
    /* Update Z axis steps/mm. */
...
void st_plan_next_move(void)
{
    ...
    {
        printString("[st_plan_next_move(): no more plan block]\r\n");
    }
    else
    {
...
void st_execute_next_step(void)
{
    ...
    if (
        ...
        (hal_motor_get_state(stepper_info.tool) == HAL_MOTOR_DRIVEN)
    )
    {
        return;
    }
    ...
    {
        /* We are done, plan next move.*/
        st_plan_next_move();
        return;
    }
    
    
    e2 = 2 * stepper_info.error;
    if (e2 >= stepper_info.dy)
    {
        if (stepper_info.x0 == stepper_info.x1)
        {
            /* We are done, plan next move.*/
            if (stepper_info.z0 == stepper_info.z1)
            {
                st_plan_next_move();
                return;
            }
    ...
    {
        if (stepper_info.y0 == stepper_info.y1)
        {
            /* We are done, plan next move.*/
            if (stepper_info.z0 == stepper_info.z1)
            {
    ...
    /* Execute Z movement if required. */
    if (stepper_info.z0 != stepper_info.z1)
    {
        /* Update z0. */
...
{  
    ...
    if (pl_block == NULL) {
        ...
        /* Initialize our Bresenham tracer. */
        //snprintf(dbg, 256, "Initialize Bresenham parameters.\r\n");
        //printString(dbg);
        ...
        stepper_info.y1 = pl_block->steps[Y_AXIS];
        stepper_info.z1 = pl_block->steps[Z_AXIS];
        stepper_info.sx = 1;
        stepper_info.sy = 1;
        stepper_info.sz = 1;
        ...
        {
            stepper_info.y1 = -stepper_info.y1;
            stepper_info.sy = -1;
        }
        
        if (pl_block->direction_bits & (1 << Z_DIRECTION_BIT))
        {
            if (selected_tool == 0)
            {
...
#if 0
        /* Shall we move the Y axis ? */
        if (pl_block->steps[Y_AXIS] > 0)
        {
            //printString("[st_prep_buffer(): move Y axis]\r\n");
            hal_motor_step(
                &HAL_MOTOR_Y,
                pl_block->steps[Y_AXIS],
                ((pl_block->direction_bits & (1 << Y_DIRECTION_BIT))!=0)?HAL_MOTOR_DIR_CW:HAL_MOTOR_DIR_CCW
            ); 
        }
#endif
    ...
    do {
      switch (prep.ramp_type) {
        ...
        default: // case RAMP_DECEL:
          ...
          // Othest_next_block_indexrwise, at end of block or end of forced-deceleration.
