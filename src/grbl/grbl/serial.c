...
{
    uint32_t tail = serial_tx_buffer_tail;
    int count = 0;
    
    /* Don't deal with more bytes than we actually have. */
    if (size > serial_get_tx_buffer_count()) {
        size = serial_get_tx_buffer_count();
    }

    if (size > 0) {
        /* Extract pending bytes from TX buffer. */
        while ((tail != serial_tx_buffer_head) && (count < size)) {
            
            /* Copy byte to p_buffer, increment tail. */
            *(p_buffer++) = serial_tx_buffer[tail++];
            count++;
            
            /* Check if tail reaches the end of Tx buffer. */
            if (tail == TX_RING_BUFFER) {
                /* Go back to the start of Tx buffer. */
                tail = 0;
            }
